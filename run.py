...
def train_ppo(args):
    ...
    if args.env == "Pendulum-v1":
        ...
        raise ValueError(
            "Unrecognized environment, please specify the hyperparameters first."
        )
...
def main(args):
    ...
    if args.alg == "PPO":
        ...
        raise ValueError(
            f"Algorithm {args.alg} not defined; options are reinforce or PPO."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg", dest="alg", type=str, default="reinforce"
    )  # Formal name of our algorithm
    parser.add_argument(
        "--seed", dest="seed", type=int, default=None
    )  # An int for our seed
    parser.add_argument(
        "--env", dest="env", type=str, default=""
    )  # Formal name of environment
